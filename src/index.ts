...
const server = new McpServer(
  ...
  },
  {
    instructions: [
      ...
      "2. Pick a provider from the **Ready to use** list returned by `opencode_setup`, then call `opencode_provider_models` to see its models and thinking/effort variants.",
      "3. IMPORTANT: Always pass `providerID` and `modelID` from the discovered providers when sending prompts, or you may get empty responses. Do NOT assume any specific provider is available — always discover first. Pass `variant` only from that model's listed keys on `opencode_provider_models`. Omit `variant` for the model default.",
      "",
      ...
      "- `opencode_provider_models` — list models and thinking/effort variant keys for a specific provider (read-only)",
